...
class SlackAlerting(BaseAlerting):
    ...
    def __init__(self, alert_config: dict):
        ...
        if not self.webhook_url:
            self.logger.error(
                "Slack Alerter configuration is missing required fields."
            )
            raise ValueError(
                "Slack Alerter configuration is missing required fields."
            )
    ...
    def notify_alerts(self):
        ...
        alerts = Alert.objects.filter(
            Q(notified_status__slack=False)
            | ~Q(notified_status__has_key="slack")
        )
        ...
        for alert in alerts:
            try:
                ...
                self.logger.exception(
                    f"Slack alert failed for {alert.name}: {str(e)}"
                )
