...
def users_pie_chart(request):
    ...
    result = {
        "no_risk": User.objects.filter(updated__range=(start, end), risk_score="No risk").count(),
        "low": User.objects.filter(updated__range=(start, end), risk_score="Low").count(),
        "medium": User.objects.filter(updated__range=(start, end), risk_score="Medium").count(),
        "high": User.objects.filter(updated__range=(start, end), risk_score="High").count(),
    }
...
def world_map(request):
    ...
    for code, name in countries.items():
        alerts = Alert.objects.filter(
            login_raw_data__timestamp__range=(start.isoformat(), end.isoformat()),
            login_raw_data__country=name,
        )
        for alert in alerts:
            key = (alert.login_raw_data["country"], alert.login_raw_data["lat"], alert.login_raw_data["lon"])
            if key not in seen:
                seen.add(key)
                result.append({
                    "country": code,
...
def alerts_line_chart(request):
    ...
    if delta.days < 1:
        ...
        result.update(aggregate_alerts_interval(start, end, timedelta(hours=1), "%Y-%m-%dT%H:%M:%SZ"))
    elif delta.days <= 31:
        result["Timeframe"] = "day"
        result.update(aggregate_alerts_interval(start, end, timedelta(days=1), "%Y-%m-%d"))
    else:
        result["Timeframe"] = "month"
        result.update(aggregate_alerts_interval(start, end, relativedelta(months=1), "%Y-%m"))
...
def user_login_timeline(request, id):
    ...
    logins = Login.objects.filter(user=user, timestamp__range=(start, end)).values_list("timestamp", flat=True)
