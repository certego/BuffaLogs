...
def risk_score_api(request, id):
    ...
    """
    user = get_object_or_404(User, pk=id)
    return JsonResponse({"username": user.username, "risk_score": user.risk_score})
...
def user_device_usage_api(request, id):
    ...
    devices = Login.objects.filter(
        user=user,
        timestamp__range=(start_date, end_date)
    ).values("user_agent").annotate(count=Count("id"))

    return JsonResponse({"devices": {d["user_agent"]: d["count"] for d in devices}})
...
def user_login_frequency_api(request, id):
    ...
    daily_counts = { (start_date + timedelta(days=i)).date(): 0 for i in range(total_days) }

    for login in Login.objects.filter(user=user, timestamp__range=(start_date, end_date)):
        daily_counts[login.timestamp.date()] += 1

    return JsonResponse({"daily_logins": [{"date": str(date), "count": count} for date, count in daily_counts.items()]})
...
def user_time_of_day_api(request, id):
    ...
    hourly_data = [{"hour": h, "weekdays": [counts[h].get(d, 0) for d in range(7)]} for h in range(24)]
...
def user_geo_distribution_api(request, id):
    ...
    counts = {name_to_code.get(cd["country"].lower(), "unknown"): cd["count"] for cd in country_data if cd["country"]}
...
def _parse_date_range(request):
    ...
    if is_naive(start): start = make_aware(start)
    if is_naive(end): end = make_aware(end)
