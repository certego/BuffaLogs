...
def match_filters(alert: Alert, app_config: Config) -> Alert:
    ...
    if app_config.ignore_mobile_logins and alert.login_raw_data["agent"]:
        ...
        if (
            ua_parsed.os
            and ua_parsed.os.family
            in settings.CERTEGO_BUFFALOGS_MOBILE_DEVICES
        ):
            logger.debug(
...
def _update_users_filters(
    db_alert: Alert, app_config: Config, db_user: User
) -> Alert:
    ...
    if (
        UserRiskScoreType.compare_risk(
            threshold=app_config.alert_minimum_risk_score,
            value=db_user.risk_score,
        )
