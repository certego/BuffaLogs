...
def update_risk_level(
    ...
    with transaction.atomic():
        ...
        if config_threshold_comparison in [
    ...
    for login in fields:
        ...
        if login["lat"] and login["lon"]:
            if Login.objects.filter(
                user_id=db_user.id, index=login["index"]
            ).exists():
                agent_alert = False
                country_alert = False
                if login["agent"]:
                ...
                if not db_user.usersip_set.filter(ip=login["ip"]).exists():
                    ...
                    logger.info(
                        f"Calculating impossible travel: {login['id']}"
                    )
            ...
            logger.info(
                f"No latitude or longitude for User {db_user.username}"
            )


def check_country(
    db_user: User, login_field: dict, app_config: Config
) -> dict:
    """
    ...
    # check "Atypical Country" alert
        ...
        - db_user.login_set.filter(country=login_field["country"])
        .last()
        .timestamp
...
def check_new_device(db_user: User, login_field: dict) -> dict:
    ...
    if db_user.login_set.filter(user_agent=login_field["agent"]).count() == 0:
        ...
        alert_info["alert_desc"] = (
            f"{AlertDetectionType.NEW_DEVICE.label} for User: {db_user.username}, at: {timestamp}"
        )
        return alert_info
