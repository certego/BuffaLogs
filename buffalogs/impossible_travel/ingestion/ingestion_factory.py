...
class IngestionFactory:
    def __init__(self):
        ...
        self.mapping = self.ingestion_config.get(
            "custom_mapping", config["elasticsearch"]["custom_mapping"]
        )
    ...
    def _read_config(self) -> dict:
        ...
        with open(
            os.path.join(
                settings.CERTEGO_BUFFALOGS_CONFIG_PATH, "buffalogs/ingestion.json"  # noqa: E501
            ),
            mode="r",
            encoding="utf-8",
        ) as f:
        ...
        if config["active_ingestion"] not in [
            ...
            raise ValueError(
                f"The ingestion source: {config['active_ingestion']} "
                "is not supported"
            )
