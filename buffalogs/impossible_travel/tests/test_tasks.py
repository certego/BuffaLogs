...
class TestTasks(TestCase):
        ...
        Alert.objects.bulk_create(
            [
                ...
                Alert(
                    user=db_user,
                    name=AlertDetectionType.NEW_COUNTRY,
                    login_raw_data="Test4",
                    description="Test_Description4",
                ),
                Alert(
                    user=db_user,
                    name=AlertDetectionType.NEW_COUNTRY,
                    login_raw_data="Test5",
    ...
    def test_process_logs_data_lost(self):
        ...
        self.assertLessEqual(
            (process_task.start_date - new_start_date_expected).total_seconds(), 5
        )
        self.assertLessEqual(
            (process_task.end_date - new_end_date_expected).total_seconds(), 5
        )
    ...
    def test_process_logs_loop(self):
        ...
        TaskSettings.objects.create(
            task_name="process_logs", start_date=start, end_date=end
        )
        ...
        process_task = TaskSettings.objects.get(task_name="process_logs")
        self.assertLessEqual(
            (start_date_expected - process_task.start_date).total_seconds(), 5
        )
        self.assertLessEqual(
            (end_date_expected - process_task.end_date).total_seconds(), 5
        )
