...
class TestSlackAlerting(TestCase):
    ...
    def test_scheduled_summary(self):
        """Test that scheduled summaries are sent correctly."""
        now = timezone.now()
        start_date = now - timedelta(days=1)
        end_date = now

        # Create some alerts for the summary
        ...
        with patch("requests.post", return_value=mock_response) as mock_post:
            self.slack_alerting.send_scheduled_summary(
                start_date=start_date,
                end_date=end_date,
                total_alerts=2,
                user_breakdown={"testuser": {"Imp Travel": 1, "New Country": 1}},
                alert_breakdown={"Imp Travel": 1, "New Country": 1},
            )
