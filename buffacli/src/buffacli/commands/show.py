...
def alert_types(
    format_option: Annotated[
        FormatOptions, typer.Option("--format", "-f", help="specify display format")
    ] = "table",
    include_description: Annotated[
        bool, typer.Option(help="Include alert types description.")
    ] = False,
):
...
def ingestion(
    ...
    active_ingestion: Annotated[
        bool, typer.Option("--active", help="Print active ingestion source")
    ] = False,
    ...
    if source:
        ...
        format_option.print(
            content=ingestion_config, title=f"Ingestion Source : {source}"
        )

    if not (source or active_ingestion):
        ingestion_sources = requests.get_ingestion_sources()
        format_option.print(
            content=Ingestion(ingestion_sources), title="Ingestion Sources"
        )
...
def alerters(
    ...
    if active_alerter:
        ...
        for resp in content:
            format_option.print(
                content=Alerters(resp), title=f"Active Alerters: {resp['alerter']}"
            )
